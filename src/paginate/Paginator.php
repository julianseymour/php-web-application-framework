...
class Paginator extends DataStructure{
	...
	public function dispose(bool $deallocate=false):void{
		...
		$this->release($this->paginatedClass, $deallocate);
	}
	...
	public function hasLimitPerPage():bool{
				...
				for ($i = 0; $i < $length; $i ++){
					...
					switch($type){
						...
						case 's':
							...
							break;
						default:
							Debug::error("{$f} invalid type specifier \"{$type}\"");
					}
				}
				if($print){
	...
	public function hasTotalPageCount():bool{
	...
	public function setTotalPageCount($total_pages){
		...
		if($total_pages < 1){
			...
			Debug::error("{$f} floor(total_pages) != total_pages");
		}elseif($print){
	...
	public function getTotalItemCount():int{
		...
		if(!$this->hasTotalItemCount()){
			Debug::error("{$f} total item count should have been calculated paginateSelectStatement for this ".$this->getDebugString());
		}
		return $this->getColumnValue("totalItemCount");
	}

	public function setTotalItemCount(int $count):int{
		$f = __METHOD__;
		$print = false;
		if($print){
	...
	public function getFullPageCount():int{
		$f = __METHOD__;
		$limit = $this->getLimitPerPage();
		if($limit === 0){
			$limit = 1;
	...
	public function setDisplayPage(int $pg):int{
		$f = __METHOD__;
		$last = $this->getLastPage();
	...
	public function setLimitPerPage(int $limit):int{
		return $this->setColumnValue('limit', $limit);
	}

	public function hasDisplayPage():bool{
	...
	public function setPaginatedClass($pc){
		if($this->hasPaginatedClass()){
			$this->release($this->paginatedClass);
		}
		return $this->paginatedClass = $this->claim($pc);
	}

	public function hasPaginatedClass():bool{
