...
class CreateRoutineStatement extends RoutineStatement implements StaticPropertyTypeInterface{
	...
	public function setRoutine(?StoredRoutine $routine): ?StoredRoutine{
		if($routine instanceof StoredRoutine){
			$this->setName(back_quote($routine->getDatabaseNameStatic()).".".back_quote($routine->getName()));
		...
		if($this->hasRoutine()){
			$this->release($this->routine);
		}
		return $this->routine = $this->claim($routine);
	}
	...
	public function dispose(bool $deallocate=false): void{
		...
		$this->release($this->routine, $deallocate);
