...
class LanguageSettingsForm extends AjaxForm
{
	...
	{
		$f = __METHOD__; //LanguageSettingsForm::getShortClass()."(".static::getShortClass().")->generateButtons({$name})";
		...
		switch ($name) {
			case DIRECTIVE_LANGUAGE:
				...
				foreach (config()->getSupportedLanguages() as $lang) {
					...
					$button->setValueAttribute(new ConcatenateCommand($name, "[", $lang, "]"));
