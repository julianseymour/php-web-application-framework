...
class FormButtonValidator extends Validator{
	...
	public function __construct(?AjaxForm $form=null){
		parent::__construct();
		if($form !== null){
			$this->setForm($form);
			...
			if(!array_key_exists("directive", $validate_me)){
				...
				if(is_array($delete_me)){
					...
					foreach($delete_me as $column_name => $child_key){
						...
						$subform = $form->getNestedForm($column_name, $struct);
					...
					if($print){
						Debug::print("{$f} successfully validated subordinate struct deletion");
					}
				}else{
