...
class UpdateElementCommand extends MultipleElementCommand{
				...
				foreach($elements as $old_id => $element){
					...
					if($element->hasSuccessors()){
						...
						foreach($successors as $s){
							...
							$final_elements[$s->getIdAttribute()] = $s;
						}
					}
				}
				...
				if($element instanceof Element){
					...
					if($this->getInnerFlag()){
						...
						if($element->hasAttributes()){
							$element->releaseAttributes($destroy);
							$element->releaseClassList($destroy);
						}
					}
				}
	...
	public function copy($that):int{
		...
		if($that->hasEffect()){
			$this->setEffect(replicate($that->getEffect()));
		}
		return $ret;
	}
	...
	public function dispose(bool $deallocate=false): void{
		...
		$this->release($this->effect, $deallocate);
	}
}
