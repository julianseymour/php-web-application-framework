...
class GetElementByIdCommand extends ElementCommand implements ValueReturningCommandInterface
{
	...
	{
		$f = __METHOD__; //GetElementByIdCommand::getShortClass()."(".static::getShortClass().")->toJavaScript()";
		$print = false;
		$id = $this->getId();
		/*
		 * if($this->hasParseType() && $this->getParseType() === TYPE_STRING){
		 *
		 * }
		 */
	...
	/*
	 * public static function declareElementById($varname, $id, $parseType=null){
	 * $get_id = new GetElementByIdCommand($id);
	 * if(isset($parseType)){
	 * $get_id->setParseType($parseType);
	 * }
	 * return DeclareVariableCommand::let(
	 * $varname,
	 * $get_id
	 * );
	 * }
	...
	public function addEventListener($type, $listener): AddEventListenerCommand
	{
		return new AddEventListenerCommand($this, $type, $listener);
	}

	public function dispatchEvent($event, ...$parameters): DispatchEventCommand
	{
