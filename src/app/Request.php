...
class Request extends Basic{
	...
	public function dispose(bool $deallocate=false):void{
		...
		if($this->hasPHPInputFileContents()){
			$this->release($this->PHPInputFileContents, $deallocate);
		}
		if($this->hasRequestURISegments()){
			$this->release($this->requestURISegments, $deallocate);
		}
		if($this->hasDirective()){
			$this->release($this->directive, $deallocate);
		}
	}
	...
	public function setAction($action){
		if($this->hasAction()){
			$this->release($this->action);
		}
		return $this->action = $this->claim($action);
	}
	...
	public function setProgressiveHyperlinkFlag(bool $value = true):bool{
		...
		return $this->setFlag('pwa', $value);
	}

	public function getProgressiveHyperlinkFlag():bool{
		return $this->getFlag('pwa');
	}
	...
	protected function hasPHPInputFileContents():bool{
		return isset($this->PHPInputFileContents);
	}
	...
	protected function getPHPInputFileContents(){
		...
		return $this->setPHPFileInputCOntents($arr);
	}
	
	public function setPHPFileInputContents($arr){
		if($this->hasPHPInputFileContents()){
			$this->release($this->PHPInputFileContents);
		}
		return $this->PHPInputFileContents = $this->claim($arr);
	}
	...
	public function setInputParameter($key, $value){
		...
		if($method === ASYNC_REQUEST_METHOD_FETCH){
			...
			}elseif(array_key_exists($key, $this->PHPInputFileContents)){
				$this->release($this->PHPInputFileContents[$key]);
			}
	...
	protected function setDirective($directive){
		if($this->hasDirective()){
			$this->release($this->directive);
		}
		return $this->directive = $this->claim($directive);
	}
	...
	public function hasRequestURISegments():bool{
		return isset($this->requestURISegments);
	}

	public function setRequestURISegments($segments){
		if($this->hasRequestURISegments()){
			$this->release($this->requestURISegments);
		}
		return $this->requestURISegments = $this->claim($segments);
	}

	public function hasRequestURISegment($i):bool{
		if(!$this->hasRequestURISegments()){
			return false;
		}
		return array_key_exists($i, $this->requestURISegments);
	}
	...
	public function getRequestURISegment($i){
		...
		}elseif(!$this->hasRequestURISegment($i)){
