...
class KeyListDatum extends JsonDatum implements ForeignKeyDatumInterface{
	...
	public function removeValue($key){
		...
		if(!isset($this->value) || !is_array($this->value)){
			Debug::error("{$f} value is undefined for this ".$this->getDebugString());
		}
		$index = array_search($key, $this->value);
		if($index === false){
			Debug::error("{$f} value \"{$key}\" is undefined for this ".$this->getDebugString());
		}
	...
	public function getPersistenceMode():int{
		...
		if(is_string($p)){
			$name = $this->getName();
			Debug::error("{$f} parent::getPersistenceMode() returned the string \"{$p}\" for column \"{$name}\"");
		}
		switch($p){
			...
			if(!empty($originals)){
				...
				if(!empty($delete_us)){
					...
					foreach($intersections as $intersection){
						...
						$query = QueryBuilder::delete()->from($db, $table)->where(
							new AndCommand(
								new WhereCondition("hostKey", OPERATOR_EQUALS), 
								new WhereCondition("relationship", OPERATOR_EQUALS), $where
							)
						)->withTypeSpecifier(
							str_pad('sss', 2 + count($delete_us), $this->getTypeSpecifier())
						)->withParameters([
	...
	public function pushValue(...$values): int{
		...
		if(!$this->hasValue()){
			...
			Debug::printArray($this->value);
			Debug::printStackTraceNoExit("{$f} returning normally");
		}
		$this->setUpdateFlag(true);
		return $pushed;
	}
